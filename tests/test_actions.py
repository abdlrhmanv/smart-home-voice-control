...
def test_core_command_arduino_mapping():
    # Swapped vs Ahmed sketch names so D12 green = music on the breadboard.
    assert map_command("light_on")["arduino"] == "MUSIC_ON"
    assert map_command("light_off")["arduino"] == "MUSIC_OFF"
    assert map_command("music_on")["arduino"] == "LIGHT_ON"
    assert map_command("music_off")["arduino"] == "LIGHT_OFF"


def test_ml_mapper_shares_core_catalog():
    mapper = CommandActionMapper()
    assert mapper.map("light_on")["arduino"] == "MUSIC_ON"
